...
divisor = 5
if args.full_fold == 0: # ! args.full_fold=1 then fake img are in eval set
  FOLDS.remove(args.fold)
  divisor = 4
#
print ('fold to use {}'.format(FOLDS))

# ! all age progression will be in same fold 
FOLDS = GetFoldFromSeedName ( sorted( list (images.keys()) ) , FOLDS)
...
for index,filename in enumerate(images): # file,age,gender,race,service_test
  ...
  for i in labelset: 
    if i in filename: 
      images_dict['label'].append(i) # ! take a string as a label
      temp_[ labelset[i] ] = 1
